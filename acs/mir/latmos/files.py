...
def check_rows_order(filename,ordersel,irows,No_error = False):
    ...
    OPTIONAL INPUTS: 
        ...
        No_error :: If True, raise no error and no print
...
def get_infos(filename):
    ...
    """
    FUNCTION NAME : get_infos()

    DESCRIPTION : Extract orders, recommended rows and utc date from observation
    ...
    OUTPUTS : 
        ...
        orders :: Name of the available order
        irow_low :: Suggested nominal lower row index
        irow_high :: Suggested nominal higher row index 
        date_utc_start :: Date and time at start of occultation
        date_utc_end :: Date and time at end of occultation

    CALLING SEQUENCE:

        orders = acs.latmos.get_orders(filename)

    MODIFICATION HISTORY : Alexandre BOEHLER (25/06/2026)
...
def extract_mult_order(filename,ordersel,irows,refalt=180.,iki_geometry=False,shift_row_edge=0):

    """
    FUNCTION NAME : extract_mult_order()

    DESCRIPTION : Extract a set of detector rows within a set of  diffraction orders 
                  from the LATMOS FITS files
    
    INPUTS : 

        filename :: Name of the input FITS file
        ordersel(norder) :: Diffraction orders
        irows(nrows) :: Index of row above the usable ones
    ...
    OUTPUTS : 
        ...
        Ls :: float, ls of the observation
        Loct :: float, local time of the observation (Mars)
        waven(order)(nwaven) :: List of Wavenumber array for each orders (cm-1)
        trans(order)(nwave,ngeom,nrows) :: List of Transmission array (3D) for each orders
        transerr(order)(nwave,ngeom,nrows) :: List of Uncertainty in transmission array for each order
        tanhe(order)(ngeom,nrows) :: List of Tangent altitude of each spectrum (km) for each order
        cont(order)(nwave,ngeom,nrows) :: List of Continuum for each order

    CALLING SEQUENCE:

        lat,lon,Ls,Loct,waven,trans,transerr,tanhe,cont = acs.latmos.extract_mult_order(filename,ordersel,irows,refalt=160.,iki_geometry=True)
    ...
    iorder = [-1]*len(ordersel)
    
    c =0
    for i in range(norders):
        if orders[i] in ordersel:
            iorder[c] = i
            c += 1
    if -1 in iorder:
        print('available orders :: ',orders)
        raise ValueError('error :: one of these order :'+ str(ordersel) +' does not exist in this observation')
    ...
    #Initialisation of the list to retrieve each order
    lat = []; lon = []; Loct = []
    mult_trans_use = []; mult_transerr = []; mult_tanhe_areoid_use = []; mult_cont_use = []
        
    for k in range(len(iorder)):
        #Re-shaping the data
        nrows_use = irow_high[iorder[k]] - irow_low[iorder[k]]   #Usable number of rows per diffraction order
        
        if np.max(irows) > nrows_use-1:
            print('warning :: some of the desired rows are not included in the order', filename,':: order', ordersel[k])
            print(np.where(irows<nrows_use)[0])
            irows = irows[np.where(irows<nrows_use)[0]]
            if len(irows)==0:
                print('irows :: ',irows)
        ...
        trans_use = np.zeros((ncols,nacq,nrowssel))
        cont_use = np.zeros((ncols,nacq,nrowssel))
        tanhe_areoid_use = np.zeros((nacq,nrowssel))
        tanhe_ellipsoid_use = np.zeros((nacq,nrowssel))
        lat_use = np.zeros((nacq,nrowssel))
        lon_use = np.zeros((nacq,nrowssel))
        ...
        if nrowssel>0:
            ...
            for irow in range(nrowssel):
                trans_use[:,:,irow] = trans[:,irow_low[iorder[k]]+irows[irow],:].T
                cont_use[:,:,irow] = cont[:,irow_low[iorder[k]]+irows[irow],:].T

                tanhe_ellipsoid_use[:,irow] = tanhe_ellipsoid_bottom[:] + tanhe_ellipsoid_per_row[:] * (irow_low[iorder[k]] + irows[irow] - irow_slitbottom[iorder[k]])
                tanhe_areoid_use[:,irow] = tanhe_areoid_bottom[:] + tanhe_areoid_per_row[:] * (irow_low[iorder[k]] + irows[irow] - irow_slitbottom[iorder[k]])
                lat_use[:,irow] = lat_bottom[:] + lat_per_row[:] * (irow_low[iorder[k]] + irows[irow] - irow_slitbottom[iorder[k]])
                lon_use[:,irow] = lon_bottom[:] + lon_per_row[:] * (irow_low[iorder[k]] + irows[irow] - irow_slitbottom[iorder[k]])
                lst_use[:,irow] = lst_bottom[:] + lst_per_row[:] * (irow_low[iorder[k]] + irows[irow] - irow_slitbottom[iorder[k]])
        ...
        for irow in range(nrowssel):
            ...
            transerr_top[inotnan_wave,irow] = np.interp(waven[iorder[k],inotnan_wave],waven[iorder[k],inotnan_error],smoothed[inotnan_error])
        ...
        mult_trans_use.append(trans_use)
        mult_cont_use.append(cont_use)
        mult_transerr.append(transerr)
        mult_tanhe_areoid_use.append(tanhe_areoid_use)
        
    lat = lat_averow[ival]
    lon = lon_averow[ival]
    Loct = lst_averow[ival]
    
    return lat,lon,Ls,Loct,waven[iorder,:],mult_trans_use,mult_transerr,mult_tanhe_areoid_use, mult_cont_use